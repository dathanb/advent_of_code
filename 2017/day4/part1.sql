...
/*
Populate a temporary table with (id, word) for each word in passphrase(id)
Then select from the temporary table where the count of distinct words equals the total count of words, grouped by passphrase id
*/

create temporary table words(id integer, word text);
...
select count(*)
from (
  select count(distinct words) as distinct_words,
  count(words) as total_words
  from words
  group by id
) as i
