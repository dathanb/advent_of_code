drop table if exists passphrases;

create table passphrases(id serial primary key, phrase text);
insert into  passphrases(phrase) values
...
-- given a string, calculate its canonical form
create or replace function to_anagram(text)
returns text language sql
as $$
  select array_to_string(array_sort(string_to_array($1, null)), '')
$$;

create temporary table words(id integer, word text, anagram text);

insert into words(id, word)
select id, unnest(string_to_array(phrase, ' ')) from passphrases;

--select array_to_string(array_sort(string_to_array('foo', null)), '');

--select to_anagram('foo'::text);

update words
set anagram = to_anagram(word);

select count(*)
from (
  select count(distinct anagram) as distinct_words,
  count(word) as total_words
  from words
  group by id
) as i
where distinct_words = total_words;
